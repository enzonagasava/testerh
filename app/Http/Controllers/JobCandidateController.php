...
class JobCandidateController extends Controller {
				...
				return datatables()->of($job_candidates)
					...
					->addColumn('action', function ($data)
					{
                        ...
                        if ($data->status != 'Aprovado') {
                            $button .= '<button type="button" name="approve" id="' . $data->id . '" class="approve btn btn-success btn-sm mr-1">Aprovar</button>';
                        }

                        if ($data->status != 'Reprovado') {
                            $button .= '<button type="button" name="reprove" id="' . $data->id . '" class="reprove btn btn-warning btn-sm mr-1">Reprovar</button>';
                        }
    ...
    public function approve($id)
    {
		...
		if ($logged_user->can('delete-job_candidate')) {
            ...
            return response()->json(['success' => __('Data is successfully approved')]);
        }
    ...
    public function reprove($id)
    {
		...
		if ($logged_user->can('delete-job_candidate')) {
            ...
            return response()->json(['success' => __('Data is successfully reproved')]);
        }
