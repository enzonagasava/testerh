...
class LeaveController extends Controller
{
    ...
    public function index()
    {
        ...
        $companies = $logged_user->companies()->get();
        ...
        if ($logged_user->can('view-leave')) {
            if (request()->ajax()) {
                ...
                if (request()->filled('start_dateFilter') && request()->filled('end_dateFilter')) {
                    $query->whereBetween('created_at', [request('start_dateFilter'), request('end_dateFilter')]);
                }

                $leave = $query->orderBy('id', 'DESC')->get();
    ...
    {
        //Se company_id em All Companies
        ...
        // Se vier company_id e department_id juntos, filtra designações por ambos
        if ($request->has('company_id') && $request->has('department_id')) {
            return response()->json([
                'type' => 'designations',
                'data' => Designation::where('department_id', $request->department_id)
        ...
        if ($employeeLeaveTypeDetail) {
            ...
            foreach ($leaveTypeUnserialize as $key => $itemArr) {
                if (in_array($request->leave_type, $itemArr)) { //leave_type = leave_type_id
                    $solicitado = (int) $request->diff_date_hidden;
                    $disponivel = (int) $itemArr['remaining_allocated_day'];

                    if ($solicitado > $disponivel) {
                        throw new Exception("Allocated quota for this leave type is less than requested total days");
                    }
                    if ($isRestore === true) {
    ...
    public function calendarableDetails($id)
    {
        if (request()->ajax()) {
            ...
            $new['Empresa'] = $data->company->company_name;
            $new['Funcionário'] = $data->employee->full_name;
            $new['Tipo de Afastamento'] = $data->LeaveType->leave_type;
            $new['Data de Início'] = $data->start_date;
            $new['Data de Término'] = $data->end_date;
            $new['Motivo da Licença'] = $data->leave_reason;
            $new['Observações'] = $data->remarks;
            $new['Status'] = 'Aprovado';
