...
class DashboardController extends Controller {
	...
	public function index()
	{
		...
		// 	->select('id', 'department_id', 'designation_id', 'is_active', 'first_name', 'last_name', 'latitude', 'longitude')
        ...
        $streetVendors = Employee::query()->when($role && $role->company_id, function($query) use ($role) {
            $query->where('company_id', $role->company_id);
        })
            ->where('is_street_vendor', 1)
            ->get();
		...
		$attendance_count = Attendance::query()->when($role && $role->company_id, function($query) use ($role) {
        ...
        $resignations_count = Resignation::query()
            ->when($role && $role->company_id, function($query) use ($role) {
                $query->where('company_id', $role->company_id);
            })
            ->where('resignation_date', now()->format('Y-m-d'))->distinct('employee_id')->count();
		...
		$total_expense_raw = FinanceExpense::query()
            ->when($role && $role->company_id, function($query) use ($role) {
                $query->where('company_id', $role->company_id);
            })
            ->sum('amount');

		$total_deposit_raw = FinanceDeposit::query()
            ->when($role && $role->company_id, function($query) use ($role) {
                $query->where('company_id', $role->company_id);
            })
            ->sum('amount');
		...
		$total_salaries_paid = Payslip::query()->when($role && $role->company_id, function($query) use ($role) {
                $query->whereHas('employee', function($query) use ($role) {
                     $query->where('company_id', $role->company_id);
                });
            })
        ...
        $municipios = Employee::query()
            ...
            ->groupBy('city')
            ->keys()
            ->map(fn($municipio) => $municipio == " " || $municipio == "" ? "N/I" : $municipio);


        $funcionariosPorMunicipio = Employee::query()
            ->whereIn('company_id', auth()->user()->companies->pluck('id'))
            ->where('is_active', 1)
            ->get()
            ->groupBy('city')
        ...
        $companies = Company::select('id', 'company_name', 'company_logo')
            ->where('id', auth()->user()->company_id) // ou use $role->company_id se preferir
            ->get();

        $data = $funcionariosPorMunicipio->map(function ($employees, $municipio) use ($companies) {
            return [
                'municipio' => $municipio ?: "N/I", // Substitui valores vazios por "N/I"
                'empresas' => $companies->map(function ($empresa) use ($employees) {
                    // Verifica se a empresa tem status ativo
                    if ($empresa->status === 'active') {
                        // Conta o número de funcionários associados à empresa no município
                        $funcionarios = $employees->filter(function ($employee) use ($empresa) {
                            return $employee->company_id === $empresa->id; // Relaciona funcionários à empresa
                        })->count();

                        return [
                            'empresa_nome' => $empresa->company_name, // Nome da empresa
                            'funcionarios' => $funcionarios // Contagem de funcionários associados
                        ];
                    }
                    return null; // Ignora empresas que não têm status ativo
                })->filter() // Remove valores nulos do resultado
            ];
        });
        ...
        foreach ($companies as $company) {
            $companiesIndicators[$company['company_name']]['employees_count'] = Employee::query()->where('company_id', $company['id'])
            ->where('is_active', 1)->whereNull('exit_date')
            ->count();
            $companiesIndicators[$company['company_name']]['street_vendors_count'] = Employee::query()->where('company_id', $company['id'])
            ->where('is_active', 1)
            ->where('is_street_vendor', 1)
            ->count();
            $companiesIndicators[$company['company_name']]['resignations_count'] = Resignation::query()->where('company_id', $company['id'])->count();
            $companiesIndicators[$company['company_name']]['leaves_count'] = Leave::query()->where('company_id', $company['id'])->count();
            $companiesIndicators[$company['company_name']]['total_expenses'] = FinanceExpense::query()->where('company_id', $company['id'])->sum('amount');
            $companiesIndicators[$company['company_name']]['total_deposits'] = FinanceDeposit::query()->where('company_id', $company['id'])->sum('amount');
            $companiesIndicators[$company['company_name']]['employees_male'] = Employee::query()->where('gender', 'Male')->where('company_id', $company['id'])
            ->where('is_active', 1)->whereNull('exit_date')
            ->count();
            $companiesIndicators[$company['company_name']]['employees_female'] = Employee::query()->where('gender', 'Female')->where('company_id', $company['id'])
            ->where('is_active', 1)->whereNull('exit_date')
            ->count();
            $companiesIndicators[$company['company_name']]['employees_physically_disabled'] = Employee::query()->where('is_physically_disabled', 1)->where('company_id', $company['id'])
            ->where('is_active', 1)->whereNull('exit_date')
            ->count();
        }


        $qualificationsCounts = [];
        $educationalLevels = QualificationEducationLevel::query()->get();
        foreach($educationalLevels as $educationalLevel) {
            $qualificationsCounts[$educationalLevel->name] = Employee::query()
            ->whereHas('employeeQualifications', function ($query) use ($educationalLevel) {
                $query->where('education_level_id', $educationalLevel->id);
            })
            ->count();
        }

        $qualificationsCounts = collect($qualificationsCounts);
        $qualificationsCountsColors = [];

        foreach ($qualificationsCounts->keys()->values()->toArray() as $key) {
            $red = rand(0, 255);
            $green = rand(0, 255);
            $blue = rand(0, 255);
            $qualificationsCountsColors[] = "rgb($red, $green, $blue)";
        }
		...
		$ticket_count = SupportTicket::where('ticket_status', 'aberto')->count();

		return view('dashboard.admin_dashboard', compact('employees', 'streetVendors', 'attendance_count', 'resignations_count', 'leave_count', 'total_expense_raw', 'total_deposit_raw', 'total_expense', 'total_deposit', 'total_salaries_paid',
            ...
            'versionUpgradeData', 'companiesIndicators', 'municipios', 'funcionariosPorMunicipio', 'data', 'qualificationsCounts', 'qualificationsCountsColors'
        ));
	}
	...
	public function profile()
	{
		...
		} else
		{
			...
			return view('profile.employee_profile', compact('user', 'employee', 'statuses',
				'countries', 'document_types', 'education_levels', 'language_skills', 'general_skills', 'relationTypes', 'salary_basics', 'usertype'));
		}
	}
	...
	public function employeeDashboard(Request $request)
	{
		...
		{
			$query->where('ticket_status', '=', 'aberto')->select('id', 'subject', 'ticket_code', 'ticket_status');
		},])
			->where('employee_id', $employee->id)->get();
