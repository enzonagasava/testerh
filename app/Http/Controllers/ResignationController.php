...
class ResignationController extends Controller {
	...
	public function index()
	{
		...
		$companies = company::select('id', 'company_name')->when($role && $role->company_id, function($query) use ($role) {
            $query->where('id', $role->company_id);
        })->get();
		...
		if ($logged_user->can('view-resignation'))
		{
			if (request()->ajax())
			{
                $resignations = Resignation::with('company', 'employee', 'department')
