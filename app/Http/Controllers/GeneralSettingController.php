...
class GeneralSettingController extends Controller
{
	...
	public function index()
	{
		if (auth()->user()->can('view-general-setting'))
		{
            ...
            Log::info('Acesso as configurações gerais: ', ['data' => $general_settings_data]);
			return view('settings.general_settings.index', compact('general_settings_data', 'zones_array', 'accounts'));
		}

        Log::warning('Tentativa de acesso não autorizado às configurações gerais');
	...
	public function update(Request $request, $id)
	{
		...
		if (auth()->user()->can('store-general-setting'))
		{
            ...
            Log::info('Configurações gerais atualizadas com sucesso', ['data' => $general_setting]);
	...
	public function mailSettingStore(Request $request)
	{
		...
		if (auth()->user()->can('view-mail-setting')) {
            ...
            Log::info('Configurações de e-mail atualizadas com sucesso', [
        ...
        Log::warning('Tentativa de acesso não autorizado às configurações de e-mail');
		return abort('403', __('You are not authorized'));
	}

    //Função que limpa o banco de dados - Desabilitada no momento
	...
	// 	return redirect()->back()->with('msg', 'Database cleared successfully');
        Log::info('Tentativa de limpar o banco de dados');
	}
