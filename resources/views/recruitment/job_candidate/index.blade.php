...
@section('content')
    ...
    <div id="confirmApproveModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title">Aprovar Candidato</h2>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body">
                    <h4 align="center">Você tem certeza que deseja aprovar este candidato?</h4>
                </div>
                <div class="modal-footer">
                    <button type="button" name="ok_approve_button" id="ok_approve_button" class="btn btn-danger">Aprovar</button>
                    <button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button>
                </div>
            </div>
        </div>
    </div>
    <div id="confirmReproveModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title">Reprovar Candidato</h2>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body">
                    <h4 align="center">Você tem certeza que deseja reprovar este candidato?</h4>
                </div>
                <div class="modal-footer">
                    <button type="button" name="ok_reprove_button" id="ok_reprove_button" class="btn btn-danger">Reprovar</button>
                    <button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    (function($) {
        ...
        $(document).on('click', '.reprove', function () {
            approve_id = $(this).attr('id');
            $('#confirmReproveModal').modal('show');
        });
        ...
        $('#ok_approve_button').on('click', function () {
            let target = "{{ route('job_candidates.index') }}/" + approve_id + '/approve';
            $.ajax({
                method: 'PUT',
                url: target,
                beforeSend: function () {
                    $('#ok_button').text('Aprovando...');
                },
                success: function (data) {
                    let html = '';
                    if (data.success) {
        ...
        $('#ok_reprove_button').on('click', function () {
            let target = "{{ route('job_candidates.index') }}/" + approve_id + '/reprove';
            $.ajax({
                method: 'PUT',
                url: target,
                beforeSend: function () {
                    $('#ok_button').text('Reprovando...');
                },
                success: function (data) {
                    let html = '';
                    if (data.success) {
                    ...
                    setTimeout(function () {
                        $('#general_result').html(html).slideDown(300).delay(5000).slideUp(300);
                        $('#confirmReproveModal').modal('hide');
                        $('#job_candidate-table').DataTable().ajax.reload();
                    }, 2000);
                }
            })
