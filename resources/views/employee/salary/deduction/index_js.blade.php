   ...
   var table_table = $('#deduction-table').DataTable({
    ...
    $('#create_deduction_record').click(function () {
        ...
        $('#deduction_action_button').val('{{trans('Add')}}');
        $('#deduction_action').val('{{trans('Add')}}');
        $('#DeductionformModal').modal('show');
    });

    $('#deduction_sample_form').on('submit', function (event) {
        event.preventDefault();
        if ($('#deduction_action').val() == '{{trans('Add')}}') {

            var fieldName = 'deduction_amount'
        ...
        if ($('#deduction_action').val() == '{{trans('Edit')}}') {
            $.ajax({
    ...
    $(document).on('click', '.deduction_edit', function () {
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $('#deduction_action_button').val('{{trans('Edit')}}');
                $('#deduction_action').val('{{trans('Edit')}}');
                $('#DeductionformModal').modal('show');
            }
        })
    });
    ...
    $('.deduction-ok').click(function () {
        ...
        $.ajax({
            url: target,
            beforeSend: function () {
                $('.deduction-ok').text('{{trans('Deleting...')}}');
            },
            success: function (data) {
                setTimeout(function () {
                    $('.confirmModal').modal('hide');
