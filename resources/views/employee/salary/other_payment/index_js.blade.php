   ...
   var table_table = $('#other_payment-table').DataTable({
    ...
    $('#create_other_payment_record').click(function () {
        ...
        $('#other_payment_action_button').val('{{trans('Add')}}');
        $('#other_payment_action').val('{{trans('Add')}}');
        $('#OtherPaymentformModal').modal('show');
    });

    $('#other_payment_sample_form').on('submit', function (event) {
        event.preventDefault();
        if ($('#other_payment_action').val() == '{{trans('Add')}}') {

            var fieldName = 'other_payment_amount'
    ...
    $(document).on('click', '.other_payment_edit', function () {
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $('#other_payment_action_button').val('{{trans('Edit')}}');
                $('#other_payment_action').val('{{trans('Edit')}}');
                $('#OtherPaymentformModal').modal('show');
            }
        })
    });
    ...
    other_payment_delete_id = $(this).attr('id');
        ...
        $('.other_payment-ok').text('{{trans('OK')}}');
    });
    ...
    $('.other_payment-ok').click(function () {
        ...
        $.ajax({
            url: target,
            beforeSend: function () {
                $('.other_payment-ok').text('{{trans('Deleting...')}}');
            },
            success: function (data) {
                setTimeout(function () {
                    $('.confirmModal').modal('hide');
