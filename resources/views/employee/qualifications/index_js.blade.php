    ...
    var table_table = $('#qualification-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('qualifications.show',$employee->id) }}",
        },
    ...
    $('#create_qualification_record').click(function () {
        ...
        $('#qualification_action_button').val('{{trans('Add')}}');
        $('#qualification_action').val('{{trans('Add')}}');
        $('#QualificationformModal').modal('show');
    });

    $('#qualification_sample_form').on('submit', function (event) {
        event.preventDefault();
        if ($('#qualification_action').val() == '{{trans('Add')}}') {
        ...
        if ($('#qualification_action').val() == '{{trans('Edit')}}') {
            $.ajax({
    ...
    $(document).on('click', '.qualification_edit', function () {
        ...
        var target = "{{ route('qualifications.index') }}/" + id + '/edit';
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $('#qualification_action_button').val('{{trans('Edit')}}');
                $('#qualification_action').val('{{trans('Edit')}}');
                $('#QualificationformModal').modal('show');
            }
        })
    });
    ...
    qualification_delete_id = $(this).attr('id');
        ...
        $('.qualification-ok').text('{{trans('OK')}}');
    });
    ...
    $('.qualification-ok').click(function () {
        let target = "{{ route('qualifications.index') }}/" + qualification_delete_id + '/delete';
        $.ajax({
            url: target,
            beforeSend: function () {
                $('.qualification-ok').text('{{trans('Deleting...')}}');
            },
            success: function (data) {
                setTimeout(function () {
                    $('.confirmModal').modal('hide');
