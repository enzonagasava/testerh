            ...
            <ul id="side-main-menu" class="side-menu list-unstyled mb-5">
                ...
                @can('view-details-employee')
                    <li class="has-dropdown {{ (request()->is('staff*')) ? 'active' : '' }}">
                        <a href="#employees" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-user-group"></i><span>{{trans('Employees')}}</span></a>
                ...
                <li class="has-dropdown {{ (request()->is('organization*')) ? 'active' : '' }}"><a href="#Organization" aria-expanded="false" data-toggle="collapse">
                        <i
                                class="dripicons-view-thumb"></i><span>{{trans('Organization')}}</span></a>
                    <ul id="Organization" class="collapse list-unstyled ">
                        @can('view-location')
                            <li id="location"><a href="{{route('locations.index')}}">{{trans('Location')}}</a></li>
                        @endcan
                        @can('view-company')
                            <li id="company"><a href="{{route('companies.index')}}">{{trans('Company')}}</a></li>
                        @endcan
                        @can('view-department')
                            <li id="department"><a
                                        href="{{route('departments.index')}}">{{trans('Department')}}</a>
                            </li>
                        @endcan

                        @can('view-designation')
                            <li id="designation"><a
                                        href="{{route('designations.index')}}">{{trans('Designation')}}</a>
                            </li>
                        @endcan

                        {{-- @can('announcement') --}}
                            <li id="announcements"><a href="{{route('announcements.index')}}">{{trans('Announcements')}}</a></li>
                        {{-- @endcan --}}
                ...
                <li class="has-dropdown {{ (request()->is('core_hr*')) ? 'active' : '' }}">
                    ...
                    <ul id="Core_hr" class="collapse list-unstyled">
                        ...
                        @can('view-promotion')
                            <li id="promotion"><a
                                        href="{{route('promotions.index')}}">{{trans('Promotion')}}</a>
                            </li>
                        @endcan
                        @can('view-award')
                            <li id="award"><a href="{{route('awards.index')}}">{{trans('Award')}}</a></li>
                        @endcan
                        @can('view-travel')
                            <li id="travel"><a href="{{route('travels.index')}}">{{trans('Travel')}}</a></li>
                        @endcan
                        @can('view-transfer')
                            <li id="transfer"><a href="{{route('transfers.index')}}">{{trans('Transfer')}}</a>
                            </li>
                        @endcan
                        @can('view-resignation')
                            <li id="resignation"><a
                                        href="{{route('resignations.index')}}">{{trans('Resignations')}}</a>
                            </li>
                        @endcan
                        @can('view-complaint')
                            <li id="complaint"><a
                                        href="{{route('complaints.index')}}">{{trans('Complaints')}}</a>
                            </li>
                        @endcan
                        @can('view-warning')
                            <li id="warning"><a href="{{route('warnings.index')}}">{{trans('Warnings')}}</a>
                            </li>
                        @endcan
                        @can('view-termination')
                            <li id="termination"><a
                                        href="{{route('terminations.index')}}">{{trans('Terminations')}}</a>
                            </li>
                        @endcan
            ...
            @endcan
                ...
                @can('timesheet')
                    <li class="has-dropdown {{ (request()->is('timesheet*')) ? 'active' : '' }}"><a href="#Timesheets"
                            ...
                            <i class="dripicons-clock"></i><span>{{trans('Timesheets')}}</span></a>
                    ...
                    <li class="has-dropdown {{ (request()->is('payroll*')) ? 'active' : '' }}">
                        ...
                        @if(auth()->user()->can('view-payslip') || auth()->user()->can('view-paylist'))
                            <a href="#Payroll" aria-expanded="false" data-toggle="collapse">
                                <i class="dripicons-wallet"></i><span>{{trans('Payroll')}}</span>
                            </a>
                        @endif
                ...
                @endcan
                    ...
                    <li class="has-dropdown {{ (request()->is('report*')) ? 'active' : '' }}"><a href="#HR_Reports" aria-expanded="false" data-toggle="collapse">
                        ...
                        <ul id="HR_Reports" class="collapse list-unstyled ">
                            ...
                            <!--New added-->
                            <li id="attendance"><a href="{{route('attendances.index')}}">Presenças Diárias</a></li>
                    ...
                    <li class="has-dropdown {{ (request()->is('recruitment*')) ? 'active' : '' }}">
                        ...
                        @if(auth()->user()->can('view-job_post') || auth()->user()->can('view-job_candidate')|| auth()->user()->can('view-job_interview') || auth()->user()->can('view-cms'))
                            <a href="#Recruitment" aria-expanded="false" data-toggle="collapse">
                                <i class="dripicons-user-id"></i><span>{{trans('Recruitment')}}</span>
                            </a>
                        @endif
                    ...
                    <li class="has-dropdown @if(request()->is('training*')){{ (request()->is('training*')) ? 'active' : '' }}@elseif(request()->is('dynamic_variable/training_type*')){{ (request()->is('dynamic_variable/training_type*')) ? 'active' : '' }}@endif">
                        @if(auth()->user()->can('view-training') || auth()->user()->can('access-variable_type')|| auth()->user()->can('access-trainer'))
                            <a href="#Training" aria-expanded="false" data-toggle="collapse"> <i
                                        class="dripicons-trophy"></i><span>{{trans('Training')}}</span>
                            </a>
                        @endcan
                        <ul id="Training" class="collapse list-unstyled ">
                            ...
                            @can('view-trainer')
                                <li id="trainers"><a
                                            href="{{route('trainers.index')}}">{{trans('Trainers')}}</a>
                                </li>
                            @endcan
                        </ul>
                    </li>
                ...
                @can('event-meeting')
                    <li class="has-dropdown @if(request()->is('events*')){{ (request()->is('events*')) ? 'active' : '' }}@elseif(request()->is('meetings*')){{ (request()->is('meetings*')) ? 'active' : '' }}@endif">
                        ...
                        @if(auth()->user()->can('view-event') || auth()->user()->can('view-meeting'))
                            <a href="#Events_Meetings" aria-expanded="false" data-toggle="collapse"> <i
                                        class="dripicons-to-do"></i><span>{{trans('Events')}} & {{trans('Meetings')}}</span>
                            </a>
                        @endcan
                        <ul id="Events_Meetings" class="collapse list-unstyled ">
                            @can('view-event')
                                <li id="events"><a
                                            href="{{route('events.index')}}">{{trans('Events')}}</a>
                                </li>
                            @endcan
                            @can('view-meeting')
                                <li id="meetings"><a
                                            href="{{route('meetings.index')}}">{{trans('Meetings')}}</a>
                                </li>
                            @endcan
                        </ul>
                        @endcan
                        ...
                        <li class="has-dropdown {{ (request()->is('accounting*')) ? 'active' : '' }}">
                            ...
                            @if(auth()->user()->can('view-account') || auth()->user()->can('view-payee') || auth()->user()->can('view-payer') ||auth()->user()->can('view-deposit')||auth()->user()->can('view-expense')||auth()->user()->can('view-transaction')||auth()->user()->can('view-balance_transfer'))
                                <a href="#Finance" aria-expanded="false" data-toggle="collapse">
                                    <i class="dripicons-graph-pie"></i><span>{{trans('Finance')}}</span>
                                </a>
                            @endcan
                    ...
                    @if (Auth::user()->role_users_id===1)
                        <li>
                            <a href="{{url('/documentation')}}"> <i class="dripicons-information"></i><span>{{__('Documentation')}}</span></a>
                        </li>
