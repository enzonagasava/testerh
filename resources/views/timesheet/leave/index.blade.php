        ...
        <div class="row mb-3">
            <div class="col-md-2">
                ...
                <select name="filter_company" id="filter_company" class="form-control">
                    <option value="Todas as Empresas">{{ __('All Companies') }}</option>
            ...
            <div class="col-md-2">
                ...
                <select name="filter_designation" id="filter_designation" class="form-control">
            ...
            <div class="col-md-2 form-group">
                ...
                <input type="text" name="start_dateFilter" id="start_dateFilter" class="form-control date" value="">
            </div>

            <div class="col-md-2 form-group">
                <label>{{__('End Date')}} *</label>
                <input type="text" name="end_dateFilter" id="end_dateFilter" class="form-control test date" value="">
            </div>


            <div class="col-md-1 filtroButton">
                <button class="btn btn-info" type="button" id="filterLeave">Filtrar</button>
            </div>
        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 form-group">
                                <div class="custom-control custom-checkbox">
                                    ...
                                    <label class="custom-control-label"
                                           for="is_notify">{{trans('Notification')}}</label>
                                </div>
                            </div>
                            ...
                            <div class="container">
                                <div class="form-group" align="center">
                                    ...
                                    <input type="hidden" name="ticket_status" value="aberto"/>
...
<script type="text/javascript">
    ...
    (function($) {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function (data) {
                            ...
                            return data.start_date + ' {{trans('To')}} ' + data.end_date
                                + "<br>" + ' {{trans('Total')}}: ' + data.total_days + ' {{trans('Days')}} ';
                        }
                    },
                    {
                        data: 'created_at',
        ...
        $(document).on('click', '#bulk_delete', function () {
            ...
            if (id.length > 0) {
                if (confirm('{{__('Delete Selection',['key'=>trans('Leave')])}}')) {
                    $.ajax({
...
$(document).ready(function () {
    // Função para atualizar departamentos e designações ao mudar a empresa

    updateRelatedFields();
    function updateRelatedFields() {
        const companyId = $('#filter_company').val();
        console.log(companyId);
        const $department = $('#filter_department');
        const $designation = $('#filter_designation');
            $.ajax({
                url: '/fetch/related',
                type: 'GET',
    ...
    $('#filter_company').on('change', function () {
        ...
        // Se não selecionou empresa, busca todos os departamentos e designações
        if (!companyId) {
            updateRelatedFields();
            return;
        }

        // Se selecionou empresa, busca apenas os relacionados
        $.ajax({
            url: '/fetch/related',
            data: { company_id: companyId },
            type: 'GET',
            success: function (response) {
                ...
                if (response.type === 'designations') {
                    $designation.empty().append(`<option value="">${'{{ __("Select Designation") }}'}</option>`);
                    $.each(response.data, function (key, value) {
                        $designation.append(`<option value="${value.id}">${value.designation_name}</option>`);
                    });
                    $designation.selectpicker && $designation.selectpicker('refresh');
    ...
    // Atualiza designações ao mudar departamento
    $('#filter_department').on('change', function () {
        ...
        if (departmentId) {
            $.ajax({
                url: '/fetch/related',
                data: { department_id: departmentId, company_id: companyId },
                type: 'GET',
                success: function (response) {
    ...
    $('#filterLeave').on('click', function () {
        const companyId = $('#filter_company').val();
        const departmentId = $('#filter_department').val();
        const designationId = $('#filter_designation').val();
        ...
        const startDate = formatDateToYMD($('#start_dateFilter').val());
        const endDate = formatDateToYMD($('#end_dateFilter').val());


        // Constrói a URL com todos os parâmetros de filtro
        const url = `{{ route('leaves.index') }}?company_id=${companyId}&department_id=${departmentId}&designation_id=${designationId}&start_dateFilter=${startDate}&end_dateFilter=${endDate}`;

        $('#leave-table').DataTable().ajax.url(url).load();
    });
});
