            ...
            var table_table = $('#invoice-table').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            if (data == 0) {
                                return "<td><div class = 'badge badge-info'>{{trans('Unpaid')}}</div></td>";
                            } if (data == 1) {
                                return "<td><div class = 'badge badge-success'>{{trans('Paid')}}</div></td>";
                            }
                            else {
                                return "<td><div class = 'badge badge-success'>{{trans('Sent')}}</div></td>";
                            }
                        }
                    },
                    {
                        ...
                        render: function (data,type,row) {
                            if (row.status == 1) {
                                return '<div class="btn-group"><button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Change Status &nbsp;</button><div class="dropdown-menu">' +
                                    '<li data-status_id="'+0+'" data-invoice_id="'+row.id+'"  class="invoice_status">{{trans('Unpaid')}}</li>'+
                                    '</div></div>';
                            }
                            if (row.status == 2) {
                                return '<div class="btn-group"><button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Change Status &nbsp;</button><div class="dropdown-menu">' +
                                    '<li data-status_id="'+1+'" data-invoice_id="'+row.id+'"  class="invoice_status">{{trans('Paid')}}</li>'+
                                    '</div></div>';
                            }
                            return '<div class="btn-group"><button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Change Status &nbsp;</button><div class="dropdown-menu">' +
                                '<li data-status_id="'+1+'" data-invoice_id="'+row.id+'"  class="invoice_status">{{trans('Paid')}}</li><hr>'+
                                '<li data-status_id="'+2+'" data-invoice_id="'+row.id+'" class="invoice_status">{{trans('Send')}}</li>'+
                                '</div></div>';
                        }
                    }
                ],
