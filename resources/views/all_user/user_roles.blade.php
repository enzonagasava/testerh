        ...
        <div class="container-fluid mb-3">
            {{-- @can('set-permission')
                <a href="{{route('roles.index')}}" class="btn btn-info mr-1"><i
                            class="fa fa-puzzle-piece"></i> {{trans('Role')}} </a>
            @endcan --}}
            {{-- @can('assign-role')
                ...
                columns: [
                    ...
                    // }
                    {
                        ...
                        render: function (data,type,row) {
                            ...
                            if (row.role_name == 'admin' ) {
                                return 'A função de administrador não pode ser alterada';
                            }
                            else if (row.role_name == 'client') {
                                return 'A função do cliente não pode ser alterada';
                            }
                            else {
                                // return '@can('assign-role')<div class="btn-group"><button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Assign Role &nbsp;</button><div class="dropdown-menu">@foreach($roles as $role)<li data-employee_id="'+row.id+'" data-role_id="{{$role->id}}" class="assign-role">{{$role->name}}</li>@endforeach</div></div>@endcan';
                                return '@can('role-access-user')<div class="btn-group"><button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Atribuir função &nbsp;</button><div class="dropdown-menu">@foreach($roles as $role)<li data-employee_id="'+row.id+'" data-role_id="{{$role->id}}" class="assign-role">{{$role->name}}</li>@endforeach</div></div>@endcan';
